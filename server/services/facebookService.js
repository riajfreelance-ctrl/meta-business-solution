...
async function sendMessage(psid, response, accessToken) {
    const token = accessToken || PAGE_ACCESS_TOKEN;
    const body = {
        recipient: { id: psid },
        message: response
    ...
    try {
        ...
        serverLog(`Message sent to ${psid}`);
    } catch (e) {
...
async function getPostContent(postId, accessToken) {
    const token = accessToken || PAGE_ACCESS_TOKEN;
    try {
        const resp = await axios.get(`https://graph.facebook.com/v12.0/${postId}?fields=message,from&access_token=${token}`);
        return resp.data;
    } catch (e) {
...
async function getLatestPosts(pageToken, limit = 10) {
    try {
