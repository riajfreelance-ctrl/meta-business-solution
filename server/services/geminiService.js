...
const defaultModel = genAI.getGenerativeModel({ model: "gemini-1.5-flash" }, { apiVersion: 'v1' });

async function getDynamicModel(apiKey) {
    const customAI = new GoogleGenerativeAI(apiKey || process.env.GEMINI_API_KEY);
    return customAI.getGenerativeModel({ model: "gemini-1.5-flash" }, { apiVersion: 'v1' });
}
